...
The main type of behaviour is its mobility pattern,
which for the proof of concept, 
has been set to be a straight line movement.
...
form of firmware. 
Both this and the target's models were spawn
...
needs to be respawn. For this reason, the drone's
...
The reinforcement learning was done 
using another library called \gym.
...
called \texttt{Env}
which we inherited and then we overrode the 
...
in the succeeding \cref{sec:obj-detection}, it involves 
...
\textsc{id}'s, we need to also distinguish between
...
to use numbers by displaying a unique \textsc{id}
...
but this needs to be done using the libgazebo7 running
...
could not be used on our machine because libgazebo7
uses some shared libraries whose versions can be only 
be run on Ubuntu 16.04. One such culprit is the 
libprotobuf.so.9 from the Google Protocol Buffer 
(Protobuf) used heavily by Gazebo. Consequently, 
...
Having proven the use of the physics simulation
through Sphinx, Olympe, \gym and the 
\gls{drl} model, and  
the feasibility of an \gls{rl} training 
