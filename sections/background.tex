...
on a simulated 3\textsc{dr} Iris \gls{uav}.
...
% {{{{ Wang2018
% Topic sentence
Commercial drones with an onboard computer use an \textsc{sdk} with image processing techniques.
% Explanation
The hardware architecture in \citeauthor{Wang18} work for this paper includes a \textsc{dji} 
...
% {{{{ Zhao2018
% Topic sentence
The use of embedded system attached to the \gls{uav} which implements mobility pattern recognition decreases response time and saves transmission bandwidth. 
% Explanation 
\citeauthor{Zhao18} work used a quadrotor \gls{uav} supported with \gls{gps} module and a 
