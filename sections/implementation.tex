...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{ X p{12.3cm} }
        ...
        \textsc{td}-lambda
                & 
                \gls{td} learning ($\lambda$) balances the variance
                and bias very well by allocating weights to all
                actions in a trajectory according to the
                hyperparameter $\lambda$ in estimating the state-value
                function. 
                \\ \addlinespace
...
Below are the list of use cases that a user can do using the AirEye website as 
seen in \cref{fig:gui-use-cases} : 
\begin{itemize}	
