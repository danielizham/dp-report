...
The training part is divided into two sections first one is \gls{rl}, which is
used to allow the smart navigation in the \anafi drone, second part is object
detection and identification, which is used to detect, count and identify the targets
...
\begin{enumerate}
    \item connecting to the drone access point 
...
to the onboard computer using the Raspberry Pi internal 
\SI{2.4}{\giga\hertz}
Wi-Fi interface. Firstly we add the \anafi drone access point 
to the saved devices list in the Raspberry Pi and 
once the Raspberry Pi boots up, it automatically keeps 
searching for the access point, and it connects 
to it once it is available.
For the connection between the command and control system 
and the Raspberry Pi, 
Raspberry Pi will use the 
\SI[per-mode=symbol,per-symbol=p]{300}{MBps} 
Wi-Fi adapter dongle connected to 
the Raspberry Pi, through its \textsc{usb}, interface this 
will allow it to create an access point,
this access point will be used to allow command and control device 
to connect and control the Raspberry Pi,
for controlling the onboard computer \textsc{ssh} 
protocol and open the graphical interface 
control using the \textsc{vnc} feature will be used.
